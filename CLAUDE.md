...
- **成交周期(deal_cycle_days) = 成交订单的order_date - contacts.add_time（加微信日期）**
- **转化率时间筛选 = 用won订单的order_date过滤**，不用deposit日期
- deposit(amount≤1000)是看小样阶段，不算成交，不计算deal_cycle_days
- deposit(amount>1000)视同won，算成交
- **同一客户只算首次成交的周期，复购不算**（避免拉长平均周期）
- **成交单数按客户去重**（一个客户多个产品只算1单）
- **典型成交周期 = 去掉≥90天的后取平均**（去除老客户极端值影响）
- 平均成交周期 = 所有首购客户的原始平均（含极端值，小字展示）
...
- 普通会话：35轮时提醒换窗口
- 从上个窗口继续的会话：20轮时提醒
- 涉及具体事实（人名、数字、字段名）时必须查数据库/文件确认，不凭记忆回答

触发提醒后立即执行：
1. 自动把当前进展、未完成的事、关键发现写入 docs/handoff/auto-handoff-{日期}.md
2. 同步更新CLAUDE.md的"最近变更"区块
3. 告诉Woniu："窗口太长了，进展已保存，建议开新窗口。新窗口说'继续{任务代号}'即可。"
不要等Woniu发现，主动处理。
...
### 2026-03-15
- **T-032订单脏数据修复**：orders.wechat_id脏数据从222条→76条（修复128条匹配+11条垃圾置NULL）；反查成功率从87.7%→94.8%（1715/1809）
  - nickname精确匹配36种dirty_wid→103条订单更新
  - 特殊格式wxid提取（"昵称/wxid_xxx"）→6条
  - 模糊匹配群聊前缀（去"群聊："前缀）→13条
  - 垃圾数据置NULL（"无"/"日期"/"群聊无对应"）→13条
  - 剩余76条：微信号/昵称在contacts表中无记录，需人工确认
- **feishu_sync_wiki_orders.py增强**：resolve_wechat_id新增nickname唯一匹配兜底、wxid提取、垃圾值过滤，防止新脏数据产生
- **订单去重**：orders表1819→990条，删除829条重复数据（飞书同步历史遗留），加了feishu_record_id唯一索引防重复
- **成交周期完善**：deposit>1000也纳入周期计算；复购不算周期；同客户多产品按1单算
- **截尾成交周期**：新增「典型成交周期」= 去掉≥90天后取平均，前端大字展示；平均值小字展示
- **按销售漏斗数据**：缓存新增funnelBySalesCohort/funnelBySalesPeriod，每个销售单独算漏斗（前端待接入）
- **仪表盘数据审查**：计划中，逐模块用原始数据源验证
